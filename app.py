...
class Todo(db.Model):
    ...
    max_votes = db.Column(db.Integer,default = 0)
...
def admin():
    ...
    if request.method == 'POST':
        ...
        task_maxVotes = request.form['max_votes']

        print(task_content)
        print(task_timezone)
        print(task_eventDate)
        if(task_content != "" and task_timezone!= "" ):
            new_task = Todo(content=task_content,timezone=task_timezone,location=task_location,description=task_description,eventTitle=task_eventTitle,max_votes=task_maxVotes)
            print("hi")
            try:
                print("hi")
